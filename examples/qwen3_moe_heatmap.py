...
monkey_patch(modeling_qwen3_moe, verbose=True)
...
model = modeling_qwen3_moe.Qwen3MoeForCausalLM.from_pretrained(path, device_map='cuda:1', torch_dtype=torch.bfloat16) #, quantization_config=quantization_config)
...
pdf_heatmap(tokens, relevance, path='qwen3_moe_30b_heatmap.pdf', backend='xelatex') # backend='xelatex' supports more characters

# plot again without first token, because it receives large relevance values overshadowing the rest
pdf_heatmap(tokens[1:], relevance[1:] / relevance[1:].max(), path='qwen3_moe_30b_heatmap_wo_first.pdf', backend='xelatex')