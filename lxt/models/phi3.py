...
class Phi3MLP(nn.Module):
    ...
    def forward(self, hidden_states: torch.FloatTensor) -> torch.FloatTensor:
        ...
        up_states = self.proj_silu_mul(up_states, self.silu(gate))
...
PHI3_ATTENTION_CLASSES = {
    "eager": Phi3Attention,
    # -- others not supported in LXT right now --
...
class Phi3DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = lf.add2(residual, self.resid_attn_dropout(attn_outputs))
        residual = hidden_states
        hidden_states = self.post_attention_layernorm(hidden_states)
        hidden_states = self.mlp(hidden_states)
        hidden_states = lf.add2(residual, self.resid_mlp_dropout(hidden_states))
...
class Phi3Model(Phi3PreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing: # changed
