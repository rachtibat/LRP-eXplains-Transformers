...
def custom_bmm(A, B):
    """
    Implements batch matrix-matrix multiplication using torch.matmul.

    Args:
        A: Tensor of shape (batch_size, M, N)
        B: Tensor of shape (batch_size, N, P)

    Returns:
        Tensor of shape (batch_size, M, P)
    """
    batch_size, M, N = A.shape
    _, N2, P = B.shape
    assert N == N2, "Matrix dimensions must match for multiplication"

    # Using torch.matmul which supports batch-wise multiplication
    # return torch.matmul(A, B)
    return lf.matmul(A, B)  #### <---- LXT
...
class OPTAttention(nn.Module):
    ...
    def forward(
        ...
        if attn_weights.dtype == torch.float16:
            ...
            attn_weights = self.softmax(attn_weights).to(torch.float16)  #### <---- LXT
        else:
            # attn_weights = nn.functional.softmax(attn_weights, dim=-1)
            attn_weights = self.softmax(attn_weights)  #### <---- LXT
        ...
        attn_output = custom_bmm(attn_probs, value_states)  #### <---- LXT
...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig):
        ...
        # )
        self.self_attn_layer_norm = lm.LayerNormEpsilon(
            self.embed_dim, elementwise_affine=config.layer_norm_elementwise_affine
        )  #### <---- LXT
