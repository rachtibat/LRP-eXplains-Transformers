...
attnLRP = {
    ...
    torch.nn.Dropout: partial(
        patch_method, dropout_forward
    ),  # we patch dropout just in case if the user sets the model to train() mode
    modeling_bert: partial(replace_module, sys.modules[__name__]),
}
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        attention_scores = divide_gradient(
            attention_scores, 2
        )  ### <------------------------------------------- LXT
...
class BertSdpaSelfAttention(BertSelfAttention):
    ...
    def forward(
        ...
        query_layer = divide_gradient(
            query_layer, 2
        )  ### <------------------------------------------- LXT
        key_layer = divide_gradient(
            key_layer, 2
        )  ### <------------------------------------------- LXT
...
class BertIntermediate(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        hidden_states = self.dense(hidden_states)
        hidden_states = identity_rule_implicit(
...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        pooled_output = identity_rule_implicit(
